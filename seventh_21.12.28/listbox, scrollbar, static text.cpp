...
void PaintTheBlock(HWND hCtrl, int iColor, int iFigure)
{
	...
	PaintWindow(hCtrl, iColor, iFigure);				//WndProc - WM_PAINT - PaintWindow호출에서 윈도우핸들 값만 달라짐
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HSCROLL:		//스크롤은 WM 메시지를 발생시킴
		...
		PaintTheBlock(hCtrlBlock, RGB(iRed, iGreen, iBlue), iShape);			//미리보기 창 그리기 (미리보기 창(윈도우), 색깔ID, 모양 ID)
	...
	case WM_COMMAND:
		...
		case IDC_LIST1:
			...
			PaintTheBlock(hCtrlBlock, RGB(iRed, iGreen, iBlue), iShape);
			break;
		}
		break;
	case WM_PAINT:
		...
		PaintTheBlock(hCtrlBlock, RGB(iRed, iGreen, iBlue), iCurrentShape);		//없으면 최초 미리보기에 나타나지 않음
