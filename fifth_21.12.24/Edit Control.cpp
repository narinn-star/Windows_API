	...
	case WM_COMMAND:
		...
		case ID_EDIT:
			switch (HIWORD(wParam)) {
				//EN_CHANGE 주석처리하면 EDIT 클릭 시 바로 COMMAND 전달, 주석처리 안하면 EDIT 클릭 후 문자열 입력될 시 바로 COMMAND 전달
			case EN_CHANGE:
				GetWindowText(hEdit, str, 128);		//Edit control의 입력값을 읽기
				SetWindowText(hWnd, str);			//부모 윈도우 캡션에 출력
			}
		}
		return 0;
