...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		CreateWindow(TEXT("button"), TEXT("적용"), WS_CHILD | WS_VISIBLE |
			BS_PUSHBUTTON, 220, 10, 100, 25, hWnd, (HMENU)ID_BUTTON, g_hInst, NULL);				//220,10,100,25 => 위치 지정
		return 0;
	...
	case WM_COMMAND:
		...
		case ID_CIRC:
			...
			break;
		case ID_BUTTON:
			GetWindowText(hEdit, str, 128);			//EDIT Control의 값을 받아옴
			iCurrentShape = _ttoi(str);				//str을 int로 atoi의 중립코드 #include <tchar.h> 필요함
			break;
		}
