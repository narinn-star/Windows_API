...
int iCurrentColor = 0;  //default  빨강
int iCurrentShape = 0;
//*

HWND hList, hCombo;
TCHAR *Items[] = { TEXT("Rectangle"), TEXT("Ellipse") };
TCHAR *Items2[] = { TEXT("빨강"), TEXT("초록"), TEXT("파랑") };
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(hList, LB_SETCURSEL, iCurrentShape, 0);  //p.215 상단 표

		hCombo = GetDlgItem(hDlg, IDC_COMBO1);
		for (int i = 0; i<3; i++) {
			SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)Items2[i]);
		}

		SendMessage(hCombo, CB_SETCURSEL, iCurrentColor, 0);
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			iCurrentColor = SendMessage(hCombo, CB_GETCURSEL, 0, 0);
			EndDialog(hDlg, IDOK);
			return TRUE;
		case IDCANCEL:
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONUP:
		bNowDraw = FALSE;
		hdc = GetDC(hWnd);
		hBrush = CreateSolidBrush(Colors[iCurrentColor]); // 빨간 붓 선언
		...
		obj[iCount].color = Colors[iCurrentColor];
